...
function getBodyXML(chapter, book, contentEl) {
  ...
  // In https://parahumans.wordpress.com/2017/10/21/glowworm-p-1/ the Last/Next Chaper isn't in a seperate <p>
  contentEl.lastElementChild.innerHTML = contentEl.lastElementChild.innerHTML?.split('Last Chapter')[0] || contentEl.lastElementChild.innerHTML
  ...
  // glow-worm fixes
  if (book === 'glow-worm') {
    xml = xml.replace(/Using identity<\/strong> “<strong><\/strong>/ug, /Using identity “/)
    xml = xml.replace(/Forum <span style="text-decoration:underline;">thread.<\/span>/ug, "Forum <span style=\"text-decoration:underline;\">thread</span>.")
